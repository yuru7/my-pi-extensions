...
- **`ask`** は承認プロンプトを表示します。判定に使ったモデルと設定順位(`Risk Assessor:`)、リスクレベル、ユーザーの指示への適合度、実行予定の操作、AI の要約と理由が含まれます。対話 TUI ではプロンプト表示時にターミナルのベルが鳴ります。選択肢は固定で **No / Yes(初期選択は No)** のため、そのまま Enter を押してもブロックされます。Esc・Ctrl-C・UI が利用できない場合もブロック(fail closed)です。

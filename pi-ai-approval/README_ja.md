...
具体例: 報告されたバグを直すためのファイル編集 → `low`。実装に必要な依存の追加 → `medium`。独断での `git reset --hard` → `high`(明示指定があれば `medium`)。明示指示があっても本番 DB migration → `high`。本番データの一括削除 → `very_high`。指示があっても秘密情報の外部送信 → `critical`。
